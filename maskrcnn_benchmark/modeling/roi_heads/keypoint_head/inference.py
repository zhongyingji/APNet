...
def heatmaps_to_keypoints(maps, rois):
    ...
    for i in range(len(rois)):
        ...
        w = roi_map.shape[2]
        roi_reshape = roi_map.reshape(num_keypoints, -1)

        pos = roi_reshape.argmax(axis=1)

        softmax_score = np.array(F.softmax(torch.from_numpy(roi_reshape), dim=1))
        final_score = softmax_score[np.arange(num_keypoints), pos]
